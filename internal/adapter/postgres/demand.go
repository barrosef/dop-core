...
// write the event with Emit in the SAME transaction, commit (ADR-0014). There is
...
// AppendMessage writes the message to the log ONLY (ADR-0004).
//
...
// that demand's log (ADR-0004). `extra` adds to the payload what only exists
...
// that a client bug would become silent corruption (ADR-0013).
